...
def instructor_or_admin_required(function=None, redirect_field_name=REDIRECT_FIELD_NAME, login_url='login'):
    '''
    Decorator for views that checks that the logged in user is an instructor,
    redirects to the log-in page if necessary.
    '''
    actual_decorator = user_passes_test(
        lambda u: u.is_active and (u.is_instructor or u.is_staff),
        login_url=login_url,
        redirect_field_name=redirect_field_name
    )
    if function:
